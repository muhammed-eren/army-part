    ...
    <footer class="footer__section footer__bg pt-0">
        <div class="container">
        ...
        <div class="footer__bottom">
            <div class="container">
                <div >
                    ...
                    -->
                    <center>
                        <p class="copyright__content"><span class="text__secondary">© 2025</span> Powered by <a class="copyright__content--link" target="_blank" href="https://karakuzu.info">Karakuzu.info</a> .  All Rights Reserved.</p>
                    </center>
                    <!--
                ...
                <div class="row row-cols-lg-2 row-cols-md-2">
                    ...
                    <div class="col">
                        <div class="quickview__info">
                            <form action="#">
                                <h2 class="product__details--info__title mb-10">Amazon Cloud Cam Security 245</h2>
    ...
    <script>
        const baseUrl = "http://<?= $_SERVER['HTTP_HOST'] ?>/";
        ...
        $('.add-to-wishlist').on('click', function(e) {
            ...
            var userID = <?= $_SESSION['USER']['kullanici_id'] ?>;
            $.ajax({
                url: baseUrl + 'api/add-to-wishlist.php',
                method: 'POST',
                data: {
                    user_id: userID,
                ...
                success: function(response) {
                    ...
                    icon.toggleClass('fa-solid fa-heart fa-regular fa-heart'); 
                }
            });
        });
        ...
        $('.delete-wishlist').on('click', function(e) {
            ...
            var userID = <?= $_SESSION['USER']['kullanici_id'] ?>;
            $.ajax({
                url: baseUrl + 'api/delete-wishlist.php',
                method: 'POST',
                data: {
                    user_id: userID,
                    product_id: productId
                ...
                success: function(response) {
                    $('#wishlist_' + productId).css('position', 'relative').animate({
                        opacity: 0,
                        left: "-=200",
                        height: 0,
                        marginTop: 0,
                        ...
                        $('#wishlist_' + productId).slideUp(200, function() {
                            $(this).remove();
                        });
                    });
        ...
        $(document).on('click', '.quantity__value.decrease', function() {
            ...
            if(input.length && input.data('counter') !== undefined){
                let value = parseInt(input.val()) || 1;
                value = value > 1 ? value - 1 : 1;
                input.val(value);

                // Ajax ile server tarafına güncelle
                let productId = $(this).data('id');
                let userID = <?= $_SESSION['USER']['kullanici_id'] ?>;
                $.post(baseUrl + 'api/add-to-cart.php', {
                    user_id: userID,
                    product_id: productId,
                    islem: 'miktar_duzenle',
                    miktar: value
                });
            }
        });
        ...
        $(document).on('click', '.remove-from-cart', function (e) {
            ...
            $.ajax({
                url: baseUrl + 'api/add-to-cart.php',
                method: 'POST',
                data: {
                    user_id: userID,
                    product_id: productId,
        ...
        $('.add-to-cart').on('click', function(e) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    button.html('Added <i class="fa-solid fa-check"></i>');
        ...
        function updateCart() {
            $.ajax({
                url: baseUrl + 'api/select-to-cart.php',
                method: 'POST',
                data: {
                ...
                success: function(response) {
                    if (response.length == 0) {
                        ...
                        let emptyMsg = $('<div class="empty-cart" style="text-align:center; padding:20px; font-weight:bold; display:none;">Your cart is empty</div>');

                        $('.minicart__product').append(emptyMsg);
                        emptyMsg.slideDown(400);
                    } else {
                        $('#cart_count').html(response.length);

                        var htmlCode = '';
                        response.forEach(element => {
                            htmlCode += `
                                <div class="minicart__product--items d-flex" id="cart-item-${element.urun_id}">
                                    <div class="minicart__thumb">
                                        <a href="/product-details/${encodeURIComponent(element.urun_seourl)}/${element.urun_id}">
                                            <img src="${baseUrl + element.urunfoto_resimyol}" alt="${element.urun_ad}" width="60">
                                        </a>
                                    </div>
                                    <div class="minicart__text">
                                        ...
                                        <span class="color__variant">${element.kategori_ad}</span>
                                        <div class="minicart__text--footer d-flex align-items-center">
                                            <div class="quantity__box minicart__quantity">
                                                <button type="button" class="quantity__value decrease" data-id="${element.urun_id}">-</button>
                                                <label>
                                                    <input type="number" class="quantity__number" value="${element.urun_adet}" data-counter />
                                                </label>
                                                <button type="button" class="quantity__value increase" data-id="${element.urun_id}">+</button>
                                            </div>
                                            <button class="minicart__product--remove remove-from-cart" data-id="${element.urun_id}" type="button">Remove</button>
                                        </div>
                                    </div>
                                </div>`;
                        });

                        $('.minicart__product').html(htmlCode).fadeIn(400);
                    }
                }
            });
        }
