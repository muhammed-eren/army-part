...
if (isset($_POST['logoduzenle'])) {
	...
	if ($_FILES['ayar_logo']['size']>2097152) {
		header("Location:../production/genel-ayar?durum=dosyaboyutu");
	}
	...
	if (in_array($ext, $izinli_uzantilar)=== false) {
		header("Location:../production/genel-ayar?durum=dosyauzanti");
		exit;
	}
...
if (isset($_POST['urunduzenle'])) {
	...
	$update=$kaydet->execute(array(
		...
		'arac_tipi_id' => $_POST['arac_tipi_id']
		));
...
if (isset($_POST['sepeteekle'])) {
	...
	$sepetsor=$db->prepare("SELECT * FROM sepet WHERE urun_id=:id");
	...
	if ($_POST['urun_adet']>=1) {
		if($stok>0) {
			...
			if ($say == 0) {
				...
				header("Location:$gelen_url?sepet=uruneklenmis");
			}	
		} else {
			header("Location:$gelen_url?sepet=stokyok");
		}

	} elseif ($_POST['urun_adet']<=0) {
		header("Location:$gelen_url?sepet=hata");
	}	
}
...
if (isset($_POST['sepetsil'])) {
	...
	if ($kontrol) {
		header("Location:$gelen_url?sepetsil=ok");
	} else {
		header("Location:$gelen_url?sepetsil=no");
	}
}
