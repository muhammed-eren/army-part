...
<?php
    ...
    $kullanici_id = $_SESSION['USER']['kullanici_id'];

    @$KEYWORDS = '%'.$_GET['q'].'%';

    $sayibul=$db->prepare("SELECT * 
                            FROM wishlist AS w
                            LEFT JOIN urun AS u ON w.urun_id = u.urun_id
                            LEFT JOIN kategori AS k ON u.kategori_id = k.kategori_id
                            WHERE w.kullanici_id = :keywords
                            ORDER BY w.wishlist_id DESC");
    $sayibul->bindParam(':keywords', $kullanici_id, PDO::PARAM_STR);
    $sayibul->execute();
    ...
    $wishlist_sayisi=$sayibul->rowCount();

    $goruntuleme_sayisi=25;

    $sayfa_katsayisi=($sayfa*$goruntuleme_sayisi)-$goruntuleme_sayisi;

    //$wishlist_ler=$db->prepare("SELECT * FROM wishlist_ler where kategori_id =: kategori_id and wishlist__durum =: wishlist__durum order by wishlist__sira ASC limit $sayfa_katsayisi, $goruntuleme_sayisi");
    $wishlist_ler = $db->prepare("SELECT * 
                            FROM wishlist AS w
                            LEFT JOIN urun AS u ON w.urun_id = u.urun_id
                            LEFT JOIN kategori AS k ON u.kategori_id = k.kategori_id
                            LEFT JOIN urunfoto AS f ON f.urun_id = u.urun_id and f.urunfoto_id = (
    ...
    // LIMIT parametreleri integer olduğu için bindParam ile bağlamak gerek
    $wishlist_ler->bindParam(':keywords', $kullanici_id, PDO::PARAM_STR);
    $wishlist_ler->bindParam(':basla', $sayfa_katsayisi, PDO::PARAM_INT);
    $wishlist_ler->bindParam(':adet', $goruntuleme_sayisi, PDO::PARAM_INT);
    $wishlist_ler->execute();

    $sayfasayisi=ceil($wishlist_sayisi/$goruntuleme_sayisi);
    if ($sayfa > $sayfasayisi) { $sayfa=$sayfasayisi; }

           // echo "wishlist_sayisi : ".$wishlist_sayisi."</br>";
    ...
    if ($sayfa_katsayisi+$goruntuleme_sayisi > $wishlist_sayisi) {
        $sayfagoruntulenenson = $wishlist_sayisi;
    } else {
                ...
                <div class="cart__section--inner">
                    <form action="#"> 
                        ...
                        <div class="cart__table">
                            <div class="cart__table--inner">
                                <div class="cart__table--body">
                                    

                                    <?php       
                                        $wishlist_sayisi = $wishlist_ler->rowCount();
                                        if($wishlist_sayisi <= 0)
                                        {
                                            echo "<center>Ürün bulunamadı</center>";
                                        }
                                        else
                                        {
                                        while ($wishlist_lercek = $wishlist_ler->fetch(PDO::FETCH_ASSOC)) {
                                    ?>

                                    <div class="cart__table--body__items d-flex justify-content-between align-items-center border-bottom" id="wishlist_<?=$wishlist_lercek['wishlist_id']?>">
                                        <div class="cart__table--body__list border-0">
                                            <div class="cart__product d-flex align-items-center">
                                                <button class="cart__remove--btn delete-wishlist" data-id="<?=$wishlist_lercek['wishlist_id']?>" aria-label="search button" type="button">
                                                    <svg fill="currentColor" xmlns="http://www.w3.org/2000/svg"  viewBox="0 0 24 24" width="16px" height="16px"><path d="M 4.7070312 3.2929688 L 3.2929688 4.7070312 L 10.585938 12 L 3.2929688 19.292969 L 4.7070312 20.707031 L 12 13.414062 L 19.292969 20.707031 L 20.707031 19.292969 L 13.414062 12 L 20.707031 4.7070312 L 19.292969 3.2929688 L 12 10.585938 L 4.7070312 3.2929688 z"/></svg>
                                                </button>
                                                <div class="cart__thumbnail">
                                                    <a href="/product-details/<?= rawurlencode($wishlist_lercek['urun_seourl']) ?>/<?= (int)$wishlist_lercek['urun_id'] ?>"><img class="border-radius-5" src="<?=$wishlist_lercek['urunfoto_resimyol']?>" alt="cart-product"></a>
                                                </div>
                                                <div class="cart__content">
                                                    <h3 class="cart__content--title h4"><a href="/product-details/<?= rawurlencode($wishlist_lercek['urun_seourl']) ?>/<?= (int)$wishlist_lercek['urun_id'] ?>"><?=$wishlist_lercek['urun_ad']?></a></h3>
                                                    <span class="cart__content--variant"><?=$wishlist_lercek['kategori_ad']?></span>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="cart__table--body__list text-center border-0">
                                            <span class="in__stock text__secondary"><?=$wishlist_lercek['urun_stok']?></span>
                                        </div>
                                        <div class="cart__table--body__list text-right border-0">
                                            <button class="wishlist__cart--btn primary__btn add-to-cart" type="button" data-id="<?=$wishlist_lercek['urun_id']?>">Add To Cart</button>
                                        </div>
                                    </div>
                                    ...
                                    <nav class="pagination justify-content-center">
                                        <ul class="pagination__wrapper d-flex align-items-center justify-content-center">
                                            <li class="pagination__list">
                                                <?php if($sayfa-1 >= 1) { ?>
                                                <a href="?sayfa=<?php echo $sayfa-1; ?>&q=<?php echo @urlencode($_GET['q'] ?? ''); ?>" class="pagination__item--arrow link">
                                            ...
                                            ?>
                                                <li class="pagination__list">
                                                    <a href="?sayfa=<?php echo $sirano; ?>&q=<?php echo urlencode($_GET['q'] ?? ''); ?>" class="<?php echo $active; ?>">
                                                        <?php echo $sirano; ?>
                                                    </a>
                                                </li>
                                            <?php } ?>
